...
public class HouseEsServiceImpl implements HouseEsService {
    ...
    @Override
    public ElasticsearchPageDTO getHouseEntitiesByCondition(HouseConditionForm form) throws Exception {
        if (form == null) {
    ...
    private ElasticsearchPageDTO getEsPage(SearchResponse response, int pageNum, int pageSize) {
        SearchHit[] searchHits = response.getHits().getHits();
        if (searchHits == null || searchHits.length == 0) {
            return new ElasticsearchPageDTO();
        }
        ...
        return new ElasticsearchPageDTO(pageNum, pageSize, totalHits, results);
    }
